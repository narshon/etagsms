...
class ModelxCode extends CCodeModel
{
	public $tablePrefix;
	public $tableName;
	public $modelClass;
	...
	public function prepare()
	{
		...
		foreach($tables as $table)
		{
			...
			$params=array(
				...
				'rules'=>$this->generateRules($table,$className,$tableName),
				'relations'=>isset($this->relations[$className]) ? $this->relations[$className] : array(),
				'datahelper' => $this->getDataHelper($className,$tableName),
			);
			$this->files[]=new CCodeFile(
				Yii::getPathOfAlias($this->modelPath).'/'.$className.'.php',
				$this->render($templatePath.'/modelx.php', $params)
			);
		}
	}
    public function getDataHelper($className,$tableName){   
	    $datahelper = new DataHelper($className,$tableName);
		return $datahelper;
	}
	...
	public function generateRules($table,$className,$tableName)
	{
		$rules=array();
		$required=array();
		$integers=array();
		...
		if($required!==array()) {
			...
			$datahelper->setModelRules('required',implode(', ',$required));
		}
			
		if($integers!==array()) {
		    $rules[]="array('".implode(', ',$integers)."', 'numerical', 'integerOnly'=>true)";
			$datahelper->setModelRules('numerical',implode(', ',$integers), "integerOnly=>true");
		}
			
		if($numerical!==array()){
			$rules[]="array('".implode(', ',$numerical)."', 'numerical')";
			$datahelper->setModelRules('numerical',implode(', ',$numerical));
		}
		...
		if($length!==array())
		{
			foreach($length as $len=>$cols){
			  ...
			  $datahelper->setModelRules('length',implode(', ',$cols), "max=>$len");
			}
				
		}
		if($safe!==array()){
			$rules[]="array('".implode(', ',$safe)."', 'safe')";
			$datahelper->setModelRules('safe',implode(', ',$safe));
		}
		
		//search scenario
		$datahelper->setModelRules('safe',implode(', ', array_keys($table->columns)), "",'search');
