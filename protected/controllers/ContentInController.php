...
class ContentInController extends Controller
{
	/**
	...
	{       
		...
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','forumpost'),
				'users'=>array('@'),
			),
        ...
        public function actionForumpost($string){
            $stringArray = WidgetHelper::explode_assoc($string, array('_',':'));
            //post to forum message
            $contentIn = KunenaMessages::model()->postMessage($stringArray['id']);
            if($contentIn){
                $contentIn->updatePost();
            }
            else{
                echo CJSON::encode(array( 'div'=>'Not Posted'));
            }
        }
        public function actionReply($view,$token){
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['ContentIn']))
		{
			$model->attributes=$_POST['ContentIn'];
			if($model->save()){
	...
	{
		$dataProvider=new CActiveDataProvider('ContentIn');
	...
	{
		$model=new ContentIn('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ContentIn']))
			$model->attributes=$_GET['ContentIn'];
	...
	{
		$model=ContentIn::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='content-in-form')
		{
