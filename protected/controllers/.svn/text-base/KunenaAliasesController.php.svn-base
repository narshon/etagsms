...
class KunenaAliasesController extends Controller
{
	/**
	...
	{
		$model=new KunenaAliases;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['KunenaAliases']))
		{
			$model->attributes=$_POST['KunenaAliases'];
			if($model->save()){
                            if (Yii::app()->request->isAjaxRequest)
                            {
                                ...
                                $this->redirect(array('view','id'=>$model->alias));
                        }
	...
	{
		$dataProvider=new CActiveDataProvider('KunenaAliases');
	...
	{
		$model=new KunenaAliases('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['KunenaAliases']))
			$model->attributes=$_GET['KunenaAliases'];
	...
	{
		$model=KunenaAliases::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='kunena-aliases-form')
		{
